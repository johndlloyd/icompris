...
function(getQtPluginsPath _path)
  # Use QtCore as anchor because Homebrew can split Qt modules into separate
  # kegs; QtQml may resolve to qtdeclarative, while plugins live with qt base.
  get_target_property(_lib_file ${QT_MAJOR}::Core LOCATION)
  get_filename_component(_lib_dir ${_lib_file} PATH)
  get_filename_component(_lib_dir_base ${_lib_dir} PATH)

  # Try known Qt layouts in order; first hit wins.
  set(_qt_plugin_candidates
    ${_lib_dir}/qt5/plugins           # Debian/Opensuse
    ${_lib_dir}/qt/plugins            # Archlinux
    ${_lib_dir_base}/plugins          # Qt installer (non-Apple)
    ${_lib_dir_base}/../plugins       # Qt installer (Apple frameworks)
    ${_lib_dir_base}/../../../share/qt/plugins # Homebrew Qt on macOS
    ${_lib_dir_base}/../share/qt/plugins       # Homebrew Qt (non-framework)
  )

  set(_plugin_root "")
  foreach(_candidate ${_qt_plugin_candidates})
    if(IS_DIRECTORY ${_candidate})
      set(_plugin_root ${_candidate})
      break()
    endif()
  endforeach()

  if("${_plugin_root}" STREQUAL "")
    message(FATAL_ERROR "Could not determine Qt plugins directory from Qt Qml library path: ${_lib_file}")
  endif()
...
function(installQtPlugin2 _plugin _dest_dir _lib)
  ...
  if(IS_DIRECTORY ${_qt_plugin_path}/${_plugin_subdir})
    install(DIRECTORY ${_qt_plugin_path}/${_plugin_subdir} DESTINATION ${_dest_dir}/${_plugin_subdir_root})
  else()
    message(WARNING "Qt plugin directory not found, skipping optional plugin '${_plugin_subdir}' from '${_qt_plugin_path}'")
